import RestrauntCards, { EnhancedRestrauntCards } from "./RestrauntCards";
...
const Body = () => {
  ...
  const RestrauntCardsEnhanced = EnhancedRestrauntCards(RestrauntCards);
  useEffect(() => {
    fetchData();
  }, []);
    ...
    <div className="body">
      <div className="flex flex-wrap p-1 m-3 justify-center">
        <div className="p-[10px]">
          ...
          />
          <button
            className="border border-solid px-3 py-0 ml-2 rounded-md bg-lime-200"
            onClick={() => {
              const filterData = listOfRestraunt.filter((item) =>
        ...
        <div className="p-[10px]">
          <button
            className="border border-solid px-3 py-0 ml-2 rounded-md bg-green-200"
      ...
      <div className="flex flex-wrap">
        {filteredRestraunt.map((item) => {
         return item?.info?.veg ? (
            ...
            <RestrauntCards key={item.info.id} resData={item} />
          );
        })}
      </div>
    </div>
